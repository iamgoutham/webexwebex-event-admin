 ...
 * `CMSG_`, `CMSD_`, `CMS_`, `CMSI_`, or `CMSJ_`. Portal `User.shortId` is often a bare hash — prefix
...
export function formatHostShortIdForMeetingException(
  ...
  const s = (raw ?? "").trim();
  if (!s) return "";
  const m = s.match(/^(CMSG|CMSD|CMSI|CMSJ|CMS)_(.+)$/i);
  if (m) {
 ...
 * logical Webex host (with or without `CMSG_` / `CMSD_` / `CMS_` / `CMSI_` / `CMSJ_` prefix).
 */
export function webexHostShortIdLookupCandidates(
  ...
  const m = t.match(/^(CMSG|CMSD|CMSI|CMSJ|CMS)_(.+)$/i);
