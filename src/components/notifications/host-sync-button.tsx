...
export default function HostSyncButton() {
  ...
  const runSync = async () => {
    ...
    try {
      const res = await fetch("/api/admin/hosts/sync", {
        method: "POST",
    ...
    <div className="space-y-3">
      ...
      {state.status === "success" ? (
        <div className="rounded-2xl border border-[#e5c18e] bg-[#fff9ef] p-4 text-xs text-[#6b4e3d]">
          <p className="font-semibold text-[#3b1a1f]">Sync complete!</p>
          <p className="mt-2">
            Created: {state.result.created} &bull; Updated: {state.result.updated}{" "}
            &bull; Skipped: {state.result.skipped}
          </p>
